angular.module('cargaArchivo')
  .controller('envioCargaArchivoCtrl', [
    ...
    function($state, $location, $log, $http, archivoUtilService, localStorageService, $uibModal, viewConfig) {
      ...
      scope.atras = function() {
        ...
        var direccion = $state.href('principal.anexosSip', {
      ...
      scope.onMensajePop = function() {
        var modalInstance = $uibModal.open({
          templateUrl: 'src/components/cargaArchivo/envioCargaSipArchivo/envioCargaSipArchivoListaError/envioCargaSipArchivoListaError.template.html',
          controller: 'envioCargaSipArchivoListaErrorController as envioCargaSipArchivoListaErrorCtrl',
          size: 'md',
          resolve: {
            dataModal: function() {
      ...
      function getBase64(file) {
        ...
        reader.onload = function() {
          ...
          archivoUtilService.cargaArchivoExcel(scope.dataInfo).post({}, scope.frmData.envioExcel.request, function(result) {
              ...
              if (scope.frmData.envioExcel.response.estado == 1) {
                ...
                console.log(scope.frmData.envioExcel.response.listaErrores);
                toastr.remove();
                toastr.success("Se subio con exito el documento excel", "Acción realizada", viewConfig.toastConfig);
